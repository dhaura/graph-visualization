...
OUTPUT_PATH_PREFIX="/pscratch/sd/d/dhaura/datasets/graph-viz/output/openTSNE/embeddings/YANDEX_embed"
MAX_ITER=1000
...
for i in {10..10}; do
    ...
    if [ $DATASET_SIZE -gt 1000000 ]; then
        DATASET_SIZE=$((1000000 * 2**(i-6)))
    fi

    INPUT_FILE="${INPUT_FILE_PREFIX}_${DATASET_SIZE}.txt"
    OUTPUT_FILE="${OUTPUT_PATH_PREFIX}_p_${PERPLEXITY}_size_${DATASET_SIZE}.txt"
    TEMP_SCRIPT="temp_openTSNE_tsne_viz_YANDEX_p_${PERPLEXITY}_size_${DATASET_SIZE}.sh"

    sed "s|__OUTPUT_FILE__|$OUTPUT_FILE|" ~/repos/graph-visualization/tsne/scripts/openTSNE_tsne_viz_large_scale_template.sh | \
