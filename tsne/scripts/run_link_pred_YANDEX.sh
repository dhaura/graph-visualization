...
INPUT_FILE_PREFIX="$SCRATCH/datasets/graph-viz/YANDEX/"
...
for i in {5..6}; do
    ...
    INPUT_FILE="${INPUT_FILE_PREFIX}YANDEX_size_${DATASET_SIZE}_adjacency.mtx"
    EMBED_FILE="${EMBED_FILE_PREFIX}YANDEX_embed_p_${PERPLEXITY}_size_${DATASET_SIZE}.txt"
    TEMP_SCRIPT="temp_link_pred_YANDEX_p_${PERPLEXITY}_size_${DATASET_SIZE}.sh"
